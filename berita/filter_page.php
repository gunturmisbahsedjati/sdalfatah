...
$filter = substr($actual_link, strrpos($actual_link, '/') + 1);
?>
<!DOCTYPE html>
<html lang="en">
      ...
      <div class="container" data-aos="fade-up">
        ...
        <?php if ($filter == "sekolah") { ?>
          ...
          <div class="row">
            ...
            <?php
            $sqlPengumuman = mysqli_query($myConnection, "select * from tb_berita where jenis_berita = 'sekolah' order by created_date desc");
            while ($viewPengumuman = mysqli_fetch_array($sqlPengumuman)) { ?>
              <div class="col-lg-4">
                <div class="post-box">
                  <div class="post-img"><img src="../../assets/img/<?= $viewPengumuman['foto_berita'] ?>" class="img-fluid" alt=""></div>
                  <span class="post-date"><?= hariTgl(tanggal($viewPengumuman['created_date'])) . ', ' . Indonesia2Tgl($viewPengumuman['created_date']) ?></span>
                  <h3 class="post-title"><?= $viewPengumuman['judul_berita'] ?></h3>
                  <a href="detail/<?= $viewPengumuman['slug_berita'] ?>" target="_blank" class="readmore stretched-link mt-auto"><span>Lihat detail...</span><i class="bi bi-arrow-right"></i></a>
                </div>
              </div>
            <?php } ?>
          </div>
        <?php } elseif ($filter == "prestasi") { ?>
          <header class="section-header">
            <p>Berita Prestasi</p>
          </header>
          ...
          <div class="row">
            ...
            <?php
            $sqlPengumuman = mysqli_query($myConnection, "select * from tb_berita where jenis_berita = 'prestasi' order by created_date desc");
            while ($viewPengumuman = mysqli_fetch_array($sqlPengumuman)) { ?>
              <div class="col-lg-4">
                <div class="post-box">
                  <div class="post-img"><img src="../../assets/img/<?= $viewPengumuman['foto_berita'] ?>" class="img-fluid" alt=""></div>
