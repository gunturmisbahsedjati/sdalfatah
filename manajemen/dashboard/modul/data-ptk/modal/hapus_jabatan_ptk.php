    ...
    if (mysqli_num_rows($sql) > 0) {
        ...
        <form action="setPTK" method="post" role="form" enctype="multipart/form-data" autocomplete="off">
            <div class="modal-header modal-header-hapus">
                <h5 class="modal-title"><i class="fa fa-trash-o"></i> Hapus Jabatan PTK</h5>
            </div>
            <div class="modal-body">
                <div class="mb-3">
                    <label class="form-label">Nama Jabatan</label>
                    <input type="text" class="form-control" placeholder="Nama Jabatan" disabled value="<?= $jabatan['nama_jabatan'] ?>" aria-describedby="defaultFormControlHelp" required>
                </div>
            </div>
            <div class="modal-footer">
                <input type="hidden" name="_token" value="<?= $jabatan['id_jabatan'] ?>">
                <button type="submit" name="hapusJabatan" class="btn btn-success">Hapus</button>
